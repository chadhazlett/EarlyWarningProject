...
###------------------------------------------------------
### Split sample for looking at OOS accuracy
###-----------------------------------------------------

#Choose a split year for training vs. testing purposes.
table(dat$mkl.start.1, dat$year)
splityear=1996
table(dat$mkl.start.1,dat$year<=splityear)
traindat=filter(dat, year<= splityear)
testdat=filter(dat, year>splityear & year<=2014)

coup.train <- glm(f.coup, family = binomial, data = traindat, na.action = na.exclude)
testdat$coup.p <- predict(coup.train, newdata = testdat, type = "response")

cwar.train <- glm(f.cwar, family = binomial, data = traindat, na.action = na.exclude)
testdat$cwar.p <- predict(cwar.train, newdata = testdat, type = "response")

threat.train <- glm(f.threat, family = binomial, data = traindat, na.action = na.exclude)
testdat$threat.p <- predict(threat, newdata = testdat, type = "response")

pitf.train <- glm(f.pitf, family = binomial, data = subset(traindat, pit.any.ongoing==0), na.action = na.exclude)
testdat$pitf.p <- predict(pitf.train, newdata = testdat, type = "response")

harff.train <- glm(f.harff, family = binomial, data = traindat, na.action = na.exclude)
testdat$harff.p <- predict(harff.train, newdata = testdat, type = "response")

#Original RF had 1000 trees; increase for stability when you have time.
# 20k is good for publication quality results.
rf.train <- randomForest(f.rf, data = traindat, na.action = na.exclude, 
             ntree = 5000, mtry = 3, cutoff = c(0.2,0.8)) # Params selected thru gridded search in validation stage a couple of yrs ago
testdat$rf.p <- predict(rf.train, type = "prob", newdata = testdat, na.action = na.exclude)[,2]

#Mean over the rf, Harff/"bad-regime", and elite threat.
mean.test=(1/3)*(testdat$rf.p+testdat$harff.p+testdat$threat.p)

### ROCs, Out-of-sample

rf.roc=roc(response=testdat$mkl.start.1, predictor = testdat$rf.p)
harff.roc=roc(response=testdat$mkl.start.1, predictor = testdat$harff.p)
threat.roc=roc(response=testdat$mkl.start.1, predictor = testdat$threat.p)
#pitf.roc=roc(response=testdat$mkl.start.1, predictor = testdat$pitf.p)
mean.roc=roc(response=testdat$mkl.start.1, predictor = mean.test)

#pdf("ROC_1996split.pdf",height=6, width=6)
plot(smooth(rf.roc), main=paste0("Out-of-Sample ROC curves, split at ", splityear))
lines(smooth(harff.roc), col=4)
lines(smooth(threat.roc), col=2)
lines(smooth(mean.roc), col=1, lty=2)

modelnames=c("RF","Bad-Regime","Threat","Mean")
aucs=signif(c(rf.roc$auc,harff.roc$auc,threat.roc$auc, mean.roc$auc),2)
legendtext=paste0(modelnames, ", AUC=",aucs)
legendtext
legend("bottomright", col=c(1,4,2,1), legend =legendtext, lty=c(1,1,1,2), lwd=2)
dev.off()

###--------------------------------------------------
### Trained on full sample, for latest forecasts
###---------------------------------------------------
...
dat$rf1000.p <- predict(rf, type = "prob", newdata = dat, na.action = na.exclude)[,2]
